...
class AdfsBaseBackend(ModelBackend):
    ...
    def _ms_request(self, action, url, data=None, params=None,headers=None, **kwargs):
        """
        ...
        """
        response = action(url,params=params, headers=headers,data=data, timeout=settings.TIMEOUT, **kwargs)
    ...
    def get_group_memberships_from_ms_graph(self, obo_access_token):
        ...
        response = self._ms_request(
            ...
            url=graph_url,
            data=None,
            params=self.get_group_memberships_from_ms_graph_params(),
            headers=headers,
        )
        claim_groups = []
